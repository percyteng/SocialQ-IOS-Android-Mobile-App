...
$category = 'Events';
$response = array();
if (isset($_POST['username'])&&isset($_POST['location'])&&isset($_POST['cost'])&&isset($_POST['notes'])&&isset($_POST['title'])&&isset($_POST['school'])) {
    ...
    try{
        ...
        $con = new DB_CONNECT();
        $db = $con->connect();
        $checkAct = $db->prepare("SELECT * FROM events WHERE username = :username AND location = :location AND price = :cost AND notes = :notes AND title = :title AND school = :school");
        ...
        if ($checkAct->rowCount() > 0) {
            ...
            $result = $db->prepare("INSERT INTO events(title, location, price, notes, comments, username, school) VALUES(:title, :location, :cost, :notes, :comments, :username, :school)");
            ...
            if (!empty($result)) {
                ...
                if ($result->rowCount() > 0) {
                    // successfully inserted into database
                    // echoing JSON response
                    $result = $db->prepare("INSERT INTO allposts(category, location, price, notes, comments, username, title, school) VALUES(:category, :location, :cost, :notes, :comments, :username, :title, :school)");
                    // binding parameters for mysql insertion
