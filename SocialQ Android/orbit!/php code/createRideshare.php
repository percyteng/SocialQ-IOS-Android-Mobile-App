...
if (isset($_POST['username'])&&isset($_POST['location'])&&isset($_POST['cost'])&&isset($_POST['notes'])&&isset($_POST['school'])) {
    ...
    try{
        ...
        $con = new DB_CONNECT();
        $db = $con->connect();
        $checkAct = $db->prepare("SELECT * FROM carpool WHERE username = :username AND location = :location AND price = :cost AND notes = :notes AND school = :school");
        ...
        if ($checkAct->rowCount() > 0) {
            ...
            $result = $db->prepare("INSERT INTO carpool(location, price, notes, comments, username, school) VALUES(:location, :cost, :notes, :comments, :username,:school)");
