package percy.SocialQ.SocialQ.createPosts;
...
import percy.SocialQ.SocialQ.Boards.tutorBoard;
import percy.SocialQ.SocialQ.R;

public class createTutors extends Activity implements View.OnClickListener{
    ...
    final int UO = 4;
    Button submitButton;
    EditText locationText, costText, notesText, subjectText;
    TextView name;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_create_tutors);
...
// Add request to queue
        String url = "http://percyteng.com/orbit/createTutors.php";
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            if (result == 1) {
                                ...
                                proDia.dismiss();
                                Intent i = new Intent(createTutors.this, tutorBoard.class);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.back) {
            ...
            if (subjectText.getText().toString().isEmpty() || subjectText.getText().toString().length() == 0 || subjectText.getText().toString() == null)
                subjectText.setError("Subject is required!");
            else if (subjectText.getText().toString().length()>50)
                subjectText.setError("Subject name has to be shorter than 500 chracters.");
            else if (locationText.getText().toString().length()>50)
                locationText.setError("Location has to be less than 50 chracters.");
            else if (costText.getText().toString().length()>30)
                costText.setError("Cost has to be less than 30 chracters.");
            else if (notesText.getText().toString().length()>500)
                notesText.setError("Notes has to be less than 500 chracters.");
            else {
                postVolley();
            }
        }
