package percy.SocialQ.SocialQ.Boards;
...
import percy.SocialQ.SocialQ.Comments;
import percy.SocialQ.SocialQ.R;
import percy.SocialQ.SocialQ.Signin;
import percy.SocialQ.SocialQ.createPosts.createTutors;
import percy.SocialQ.SocialQ.posts.Posts;

public class tutorBoard extends AppCompatActivity implements View.OnClickListener{
    Posts[] arrItems;
    ...
    public void searchItem(String textToSearch){
        ...
        for (int i = 0; i < fullList.size(); i++){
            if(!fullList.get(i).postName.contains(textToSearch)){
                if(!fullList.get(i).location.contains(textToSearch)) {
                    if (!fullList.get(i).price.contains(textToSearch))
...
// Add request to queue
        String url = "http://percyteng.com/orbit/getTutors.php";
        final JsonObjectRequest jsonRequest = new JsonObjectRequest
                (Request.Method.GET, url, null, new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        try {
                            JSONArray jsonResponse = response.getJSONArray("tutors");
                            for (int i = jsonResponse.length()-1; i >= 0; i--){
                                ...
                                if(school.equals(userSchool)) {
                                    ...
                                    String subject = eachActivity.getString("subject");
                                    String notes = eachActivity.getString("notes");
//                                    createLayout(username, location, price, notes, subject, school);
                                    fullList.add(new Posts(subject, location, price, username, notes, school));
                            ...
                            generateListContent();
                            adapter = new MyListAdaper(tutorBoard.this, R.layout.posts, data);
                            lv.setAdapter(adapter);
...
//                            alert("Whoops, something went wrong.");
    ...
    public void createLayout(final String name, final String location, final String price, final String notes, final String subject, final String school){
...
//        relay.setElevation(4);
        ...
        relay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent i = new Intent(tutorBoard.this, Comments.class);
                Bundle bundle = new Bundle();
                bundle.putString("category", "Tutors");
                bundle.putString("name", name);
                bundle.putString("location", location);
                bundle.putString("price", price);
                bundle.putString("notes", notes);
                bundle.putString("school", school);
                bundle.putString("titles", subject);
        ...
        if (name.equals(username) || username.equals("admin")){
            ...
            delete.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    deleteVolley(name, price, location, notes, "Tutors", school);
        ...
        TextView subjectTutor = new TextView(this);
        RelativeLayout.LayoutParams subjectTutorLayout = new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT, RelativeLayout.LayoutParams.WRAP_CONTENT);
        subjectTutorLayout.setMargins(60, 5, 0, 0);
        subjectTutorLayout.addRule(RelativeLayout.BELOW, R.id.title);
        subjectTutorLayout.addRule(RelativeLayout.RIGHT_OF, R.id.logo);
        subjectTutor.setLayoutParams(subjectTutorLayout);
        if (subject.length() < 10)
            subjectTutor.setText(subject);
        else
            subjectTutor.setText(subject.substring(0,7) + "...");
        subjectTutor.setId(R.id.nameEvents);

        relay.addView(subjectTutor);
        ...
        alertDialogBuilder
                ...
                .setPositiveButton("Login",new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog,int id) {
                        ...
                        // current activity
                        startActivity(new Intent(tutorBoard.this, Signin.class));
                        finish();
                    }
                })
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            if (result == 1) {
                                ...
                                Intent i = new Intent(tutorBoard.this, tutorBoard.class);
                                Bundle bundle = new Bundle();
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.createPost) {
            if (username.equals("Visitor")) {
                ...
                Intent i = new Intent(this, createTutors.class);
                Bundle bundle = new Bundle();
                bundle.putString("username", username);
...
//                }
//            });
            mainViewholder.thumbnail.setBackground(getResources().getDrawable(R.drawable.tutoring));
            mainViewholder.title.setText("TUTORING");
            if (getItem(position).postName.length() < 10)
                mainViewholder.eventName.setText(getItem(position).postName);
            else
                mainViewholder.eventName.setText(getItem(position).postName.substring(0,7) + "...");
            if(getItem(position).location.length()<10)
                mainViewholder.location.setText(getItem(position).location);
            else
                mainViewholder.location.setText(getItem(position).location.substring(0,8) + "...");
            if (getItem(position).price.length() < 10)
                mainViewholder.price.setText(getItem(position).price);
            else
                mainViewholder.price.setText(getItem(position).price.substring(0,7) + "...");
            ...
            mainViewholder.relay.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    bundle.putString("titles", getItem(position).postName);
                    bundle.putString("location", getItem(position).location);
                    bundle.putString("price", getItem(position).price);
            ...
            mainViewholder.delete.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    deleteVolley(getItem(position).username, getItem(position).price, getItem(position).location, getItem(position).notes, "Tutors", getItem(position).school);
